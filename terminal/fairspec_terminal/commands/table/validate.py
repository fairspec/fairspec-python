import polars as pl
from fairspec_library import load_table
...
def validate(
    ...
    """Validate a table from a local or remote path."""
    ...
    table_schema = session.task(
        "Loading schema",
        lambda: resolve_table_schema(res.tableSchema),
    )

    if not table_schema:
        table_schema = session.task(
            "Inferring schema",
            lambda: infer_table_schema_from_table(table),
        )

    def _validate() -> Report:
        errors = list(inspect_table(table, table_schema=table_schema))
        return create_report(errors)  # type: ignore[arg-type]

    report = session.task("Validating table", _validate)
