...
class TestInferHash:
    def test_computes_sha256_hash_by_default(self):
        ...
        result = infer_hash({"data": path})
        assert len(result) == 64

    def test_computes_md5_hash(self):
        path = write_temp_file("Hello, World!")
        result = infer_hash({"data": path}, hash_type="md5")
        assert len(result) == 32

    def test_computes_sha1_hash(self):
        path = write_temp_file("Hello, World!")
        result = infer_hash({"data": path}, hash_type="sha1")
        assert len(result) == 40

    def test_computes_sha512_hash(self):
        path = write_temp_file("Hello, World!")
        result = infer_hash({"data": path}, hash_type="sha512")
        assert len(result) == 128

    def test_consistent_hashes_for_same_content(self):
        path = write_temp_file("Hello, World!")
        result1 = infer_hash({"data": path})
        result2 = infer_hash({"data": path})
...
class TestInferTextual:
    ...
    def test_uses_custom_sample_bytes(self):
        ...
        assert infer_textual({"data": path}, sample_bytes=20) is True
